...
基于本项目 **travel_agent/agents/** 实际实现的业务 Skills，便于在 Claude 或 Cursor 中按意图调用对应 Agent。

每个 `SKILL.md` 的 frontmatter 同时也是插件声明，至少包含：
- `agent_name`
- `agent_module`
- `agent_class`
- 可选 `aliases`
...
**前置条件：** 已运行过 `python travel_agent/scripts/init_knowledge_base.py`。
...
1. **模型传入方式**  
   所有 Agent 使用 **`model=model`**（传入由 `create_chat_model()` 创建的统一模型实例）。  
