...
from travel_agent.utils.llm_resilience import retry_with_backoff, run_health_check as check_llm_health
...
class AligoCLI:
    ...
    async def initialize_system(self):
        ...
        with self.console.status("初始化中...", spinner="dots"):
            ...
            self.model_factory = create_model_factory(timeout=float(timeout_sec))
            self.model = self.model_factory(tier="default")
    ...
    async def process_query(self, user_input: str):
        ...
        with self.console.status("思考中...", spinner="dots"):
            try:
                execution = await self.pipeline.execute(
    ...
    def show_status(self):
        ...
        if self.redis_cache and self.redis_cache.enabled:
            ...
            memory_table.add_row(
                "Redis 缓存",
                f"命中 {cache_stats['hits']} 次 / 未命中 {cache_stats['misses']} 次 / 命中率 {cache_stats['hit_rate']*100:.1f}%"
            )
        else:
            memory_table.add_row(
                "Redis 缓存",
                "未启用（运行 redis-server 后自动开启）"
            )
...
def run_health_check_standalone() -> int:
    """
    独立执行健康检查（用于 `python travel_agent/cli.py health`）。
