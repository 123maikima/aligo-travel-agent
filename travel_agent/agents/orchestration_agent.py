...
from travel_agent.context.memory_updater import MemoryUpdater
...
class OrchestrationAgent(AgentBase):
    ...
    async def _emit_event(self, event: str, data: Dict[str, Any]):
        """Emit execution events for streaming observers."""
    ...
    def _summarize_agent_result(agent_name: str, result: Dict[str, Any]) -> str:
        """Build a compact, user-facing summary for intermediate streaming."""
        ...
        if agent_name == "event_collection":
            origin = data.get("origin") or nested.get("origin") or "未知"
            destination = data.get("destination") or nested.get("destination") or "未知"
            start_date = data.get("start_date") or nested.get("start_date") or ""
            return f"已提取行程要素：{origin} → {destination}" + (f"，{start_date}" if start_date else "")

        if agent_name == "information_query":
            summary = data.get("summary") or nested.get("summary") or data.get("message") or nested.get("message")
            return str(summary or "已完成实时信息查询")[:240]

        if agent_name == "rag_knowledge":
            answer = data.get("answer") or nested.get("answer")
            docs = data.get("retrieved_documents") or nested.get("retrieved_documents") or []
            if answer:
                return str(answer)[:240]
            return f"已检索知识库，命中 {len(docs)} 个片段"

        if agent_name == "preference":
            prefs = data.get("preferences") or nested.get("preferences")
            if isinstance(prefs, list) and prefs:
                return f"已识别 {len(prefs)} 条偏好更新"
            return "已完成偏好分析"

        if agent_name == "memory_query":
            answer = data.get("answer") or data.get("result") or data.get("content")
            return str(answer or "已完成历史记忆查询")[:240]

        if agent_name == "itinerary_planning":
            itinerary = data.get("itinerary") or nested.get("itinerary")
            if isinstance(itinerary, dict):
                title = itinerary.get("title") or "行程方案"
                duration = itinerary.get("duration")
                return f"已生成{title}" + (f"，时长 {duration}" if duration else "")
            content = data.get("content") or data.get("result") or data.get("message")
            return str(content or "已生成行程规划")[:240]

        generic = data.get("answer") or data.get("content") or data.get("result") or data.get("message")
        return str(generic or "已完成处理")[:240]
    ...
    @staticmethod
    def _group_tasks_by_priority(sorted_schedule: List[Dict]) -> List[tuple]:
        """兼容旧测试/旧调用的别名。"""
        return OrchestrationAgent._group_by_priority(sorted_schedule)
    ...
    async def _execute_single_agent(
        ...
        await self._emit_event("agent_start", {
            ...
            "previous_results": len(context.get("previous_results", [])),
            "timestamp": time.time(),
        })
        ...
        success_result = {"status": "success", "agent_name": agent_name, "data": result}
        duration_ms = round((time.perf_counter() - started_at) * 1000)
        summary = self._summarize_agent_result(agent_name, success_result)
        await self._emit_event("agent_done", {
            "agent": agent_name,
            "status": "success",
            "duration_ms": duration_ms,
            "summary": summary,
        })
        await self._emit_event("agent_summary", {
            "agent": agent_name,
            "summary": summary,
            "duration_ms": duration_ms,
        })
